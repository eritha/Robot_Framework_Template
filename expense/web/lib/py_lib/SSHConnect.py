...
class SSHConnect:
    ...
    def SSHConnect(self):
        ...
        ssh.connect(self.server_ip, port=self.server_port, username=self.username, pkey=self.key)
        self.ssh = ssh
    ...
    def ResetDB(self):
        self.SSHConnect()
        ssh = self.ssh
        command = 'mysql -p%s -u%s %s < %s' % (self.mysql['password'], self.mysql['user'], self.mysql['database'], self.mysql['reset_file'])
        command2 = 'mysql -p%s -u%s %s -e \"select count(*) from users;\"' % (self.mysql['password'],self.mysql['user'], self.mysql['database'])
        stdin, stdout, stderr = ssh.exec_command(command)
